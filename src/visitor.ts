  ...
  implements ICstNodeVisitor<void, unknown>
{
   ...
   */
  private hasExactlyTwoArguments(
  ...
  private isKeyBasedPredicate(
    ...
    try {
      ...
      const hasIndexAccess =
        predicateStr.includes(variable) &&
  ...
  private handleFilter(
    ...
    if (isMap) {
      ...
      const isKeyBased = this.isKeyBasedPredicate(
        predicate,
        variable,
        collection,
      )
  ...
  public expr(ctx: ExprCstChildren): unknown {
    ...
    if (typeof booleanCondition !== 'boolean') {
      throw new Error(
        `Ternary condition must be boolean, got ${typeof condition}`,
      )
    }
  ...
  private tryQualifiedResolution(
    baseName: string,
    dotExpressions: any[],
    context: any,
  ): unknown {
    ...
    for (const dotExpr of dotExpressions) {
      ...
      if (!dotExpr.children.OpenParenthesis) {
        ...
        if (
          dotExpr.children.Identifier &&
          dotExpr.children.Identifier.length > 0
        ) {
          identifierName = dotExpr.children.Identifier[0].image
        } else if (
          dotExpr.children.QuotedIdentifier &&
          dotExpr.children.QuotedIdentifier.length > 0
        ) {
  ...
  private handleStringsNamespace(dotExpressions: any[]): unknown {
    ...
    if (
      !firstDot.children.Identifier ||
      firstDot.children.Identifier.length === 0
    ) {
    ...
    switch (methodName) {
      ...
      default:
        throw new CelEvaluationError(
          `Unknown strings namespace method: ${methodName}`,
        )
    }
  }
  ...
  private handleStringsQuote(ctx: any): string {
    ...
    if (!ctx.arg || (ctx.args && ctx.args.length > 0)) {
      throw new CelEvaluationError(
        'strings.quote() requires exactly one argument',
      )
    }
  ...
  private handleBase64Namespace(dotExpressions: any[]): unknown {
    ...
    switch (methodName) {
      ...
      default:
        throw new CelEvaluationError(
          `Unknown base64 namespace method: ${methodName}`,
        )
    }
  }
  ...
  private handleBase64Encode(ctx: any): string {
    ...
    if (!ctx.arg || (ctx.args && ctx.args.length > 0)) {
      throw new CelEvaluationError(
        'base64.encode() requires exactly one argument',
      )
    }
  ...
  private handleBase64Decode(ctx: any): Uint8Array {
    ...
    if (!ctx.arg || (ctx.args && ctx.args.length > 0)) {
      throw new CelEvaluationError(
        'base64.decode() requires exactly one argument',
      )
    }
    ...
    switch (methodName) {
      ...
      case 'expr':
        ...
        if (dotExpressions.length === 1) {
          throw new CelEvaluationError(
            'Incomplete cel.expr namespace reference',
          )
        }
      ...
      default:
        throw new CelEvaluationError(
          `Unknown cel namespace method: ${methodName}`,
        )
    }
  }
  ...
  private handleCelBind(ctx: any): unknown {
    ...
    if (!ctx.arg || !ctx.args || ctx.args.length !== 2) {
      throw new CelEvaluationError(
        'cel.bind() requires exactly three arguments: variable name, value, and expression',
      )
    }
    ...
    if (!varName) {
      throw new CelEvaluationError(
        'cel.bind() first argument must be an identifier (variable name)',
      )
    }
  ...
  private handleCelBlock(ctx: any): unknown {
    ...
    if (!ctx.arg || !ctx.args || ctx.args.length !== 1) {
      throw new CelEvaluationError(
        'cel.block() requires exactly two arguments: steps array and result expression',
      )
    }
    ...
    try {
      ...
      if (listNode && listNode.children) {
        ...
        if (!Array.isArray(stepsResult)) {
          throw new CelEvaluationError(
            'cel.block() first argument must be an array of steps',
          )
        }
  ...
  private handleCelIndex(ctx: any): unknown {
    ...
    if (!ctx.arg || (ctx.args && ctx.args.length > 0)) {
      throw new CelEvaluationError(
        'cel.index() requires exactly one argument: the index number',
      )
    }
    ...
    if (!this.blockContext || !Array.isArray(this.blockContext)) {
      throw new CelEvaluationError(
        'cel.index() can only be used within a cel.block()',
      )
    }
    ...
    if (indexResult < 0 || indexResult >= this.blockContext.length) {
      throw new CelEvaluationError(
        `cel.index(${indexResult}) is out of bounds. Block has ${this.blockContext.length} elements`,
      )
    }
  ...
  private handleCelIterVar(ctx: any): unknown {
    ...
    if (!ctx.arg || !ctx.args || ctx.args.length !== 1) {
      throw new CelEvaluationError(
        'cel.iterVar() requires exactly two arguments: level and index',
      )
    }
    ...
    if (typeof level !== 'number' || !Number.isInteger(level) || level < 0) {
      throw new CelEvaluationError(
        'cel.iterVar() level must be a non-negative integer',
      )
    }

    if (typeof index !== 'number' || !Number.isInteger(index) || index < 0) {
      throw new CelEvaluationError(
        'cel.iterVar() index must be a non-negative integer',
      )
    }
  ...
  private handleProtoNamespace(dotExpressions: any[]): unknown {
    ...
    switch (methodName) {
      ...
      default:
        throw new CelEvaluationError(
          `Unknown proto namespace method: ${methodName}`,
        )
    }
  }
  ...
  private handleProtoHasExt(ctx: any): boolean {
    ...
    if (!ctx.arg || !ctx.args || ctx.args.length !== 1) {
      throw new CelEvaluationError(
        'proto.hasExt() requires exactly two arguments: message and extension',
      )
    }
  ...
  private handleProtoGetExt(ctx: any): unknown {
    ...
    if (!ctx.arg || !ctx.args || ctx.args.length !== 1) {
      throw new CelEvaluationError(
        'proto.getExt() requires exactly two arguments: message and extension',
      )
    }
  ...
  private getIndexSection(
    ...
    const expressions = [
      ...
      ...('identifierIndexExpression' in ctx
        ? ctx.identifierIndexExpression || []
        : []),
      ...('absoluteIndexExpression' in ctx
        ? ctx.absoluteIndexExpression || []
        : []),
    ...
    return expressions.reduce((acc: unknown, expression) => {
      ...
      if (Array.isArray(acc)) {
        ...
        if (
          indexType != CelType.int &&
          indexType != CelType.uint &&
  ...
  primaryExpression(ctx: PrimaryExpressionCstChildren) {
    ...
    if (ctx.Integer) {
      ...
      const isPositiveOverflow =
        !isMinInt64 &&
        (literal.length > MAX_INT64_STR.length ||
      ...
      if (isPositiveOverflow || isNegativeOverflow) {
        throw new CelEvaluationError(
          `Integer literal ${literal} exceeds int64 range`,
        )
      }
      ...
      const isLargePositive =
        literal.length > MAX_SAFE_INTEGER_STR.length ||
        (literal.length === MAX_SAFE_INTEGER_STR.length &&
          literal > MAX_SAFE_INTEGER_STR)
      const isLargeNegative =
        literal.startsWith('-') &&
        (literal.length > MIN_SAFE_INTEGER_STR.length ||
          (literal.length === MIN_SAFE_INTEGER_STR.length &&
            literal < MIN_SAFE_INTEGER_STR))
    ...
    if (ctx.UnsignedInteger) {
      ...
      if (
        literal.length > MAX_UINT64_STR.length ||
        (literal.length === MAX_UINT64_STR.length && literal > MAX_UINT64_STR)
      ) {
        throw new CelEvaluationError(
          `Unsigned integer literal ${literal}u exceeds maximum uint64 value`,
        )
      }
    ...
    if (ctx.HexUnsignedInteger) {
      ...
      if (
        literal.length > MAX_UINT64_HEX_STR.length ||
        (literal.length === MAX_UINT64_HEX_STR.length &&
          literal.toUpperCase() > MAX_UINT64_HEX_STR)
      ) {
        throw new CelEvaluationError(
          `Unsigned hex integer literal 0x${literal}u exceeds maximum uint64 value`,
        )
      }
  ...
  atomicExpression(ctx: AtomicExpressionCstChildren) {
    ...
    if (this.mode === Mode.has && !this.hasValidationDone) {
      ...
      if (!hasDotAccess && !hasIndexAccess) {
        throw new CelEvaluationError(
          'has() does not support atomic expressions',
        )
    ...
    const allExpressions = [
      ...(ctx.identifierDotExpression || []).map((expr) => ({
        type: 'dot',
        expr,
      })),
      ...(ctx.atomicIndexExpression || []).map((expr) => ({
        type: 'index',
        expr,
      })),
    ...
    for (const { type, expr } of allExpressions) {
      if (type === 'dot') {
        ...
        const isOptional =
          indexResult &&
          typeof indexResult === 'object' &&
          '__isOptionalIndex' in indexResult
        ...
        if (typeof result === 'function') {
          ...
          if (
            structData &&
            typeof structData === 'object' &&
            '__explicitlySetFields' in structData
          ) {
            result = result(
              structData,
              (structData as any).__explicitlySetFields,
            )
          } else {
          ...
          if (typeName === 'google.protobuf.Any') {
            ...
            (typeName.includes('google.protobuf') &&
              typeName.endsWith('Value')) ||
            (typeName.endsWith('Value') &&
              (typeName.includes('Int32') ||
                typeName.includes('Int64') ||
                typeName.includes('UInt32') ||
                typeName.includes('UInt64') ||
  ...
  absoluteIdentifierExpression(
    ctx: AbsoluteIdentifierExpressionCstChildren,
  ): unknown {
  ...
  identifierExpression(ctx: IdentifierExpressionCstChildren): unknown {
    ...
    if (ctx.identifierDotExpression && ctx.identifierDotExpression.length > 0) {
      const result = this.tryQualifiedResolution(
        identifierName,
        ctx.identifierDotExpression,
        data,
      )
  ...
  identifierDotExpression(
    ...
    if (isOptional) {
      ...
      if (
        ...
        typeof param === 'object' &&
        'hasValue' in param &&
        typeof (param as any).hasValue === 'function'
      ) {
    ...
    if (ctx.OpenParenthesis) {
      ...
      if (
        this.isCollectionMacro(identifierName) &&
        this.hasExactlyTwoArguments(ctx) &&
        this.isContextBasedCollection(param)
      ) {
  ...
  getIdentifier(searchContext: unknown, identifier: string): unknown {
    ...
    if (
      ...
      this.isTypeIdentifier(searchContext)
    ) {
    ...
    if (
      this.isProtobufWrapperField(identifier) &&
      searchContext &&
      typeof searchContext === 'object' &&
      '__hasField' in searchContext
    ) {
      ...
      if (this.mode === Mode.has) {
        ...
        if (typeof hasFieldFn === 'function') {
          ...
          if (fieldValue !== undefined) {
            ...
            if (
              identifier.startsWith('repeated_') &&
    ...
    if (value === undefined) {
      ...
      if (this.mode === Mode.has && this.isProtobufMessage(searchContext)) {
        throw new CelEvaluationError(
          `Unknown field '${identifier}' in protobuf message`,
        )
      }
  ...
  private isTypeIdentifier(identifier: string): boolean {
    ...
    return (
      typeNames.includes(identifier) || identifier.startsWith('google.protobuf')
    )
  }
  ...
  private isProtobufMessage(obj: unknown): boolean {
    ...
    return (
      ...
      typeof obj === 'object' &&
      ('__hasField' in obj ||
        'repeated_nested_enum' in obj ||
        'standalone_enum' in obj)
    )
  }
  ...
  private isProtobufExtensionField(identifier: string): boolean {
    ...
    return (
      identifier.includes('cel.expr.conformance.proto2.') &&
  ...
  private getProtobufFieldDefault(identifier: string): unknown {
    if (identifier.startsWith('single_')) {
      ...
      if (
        identifier.includes('wrapper') ||
        identifier.includes('any') ||
        identifier.includes('duration') ||
        identifier.includes('timestamp') ||
        identifier.includes('struct') ||
        identifier.includes('value')
      ) {
        return null
      } else if (
        identifier.includes('int') ||
        identifier.includes('fixed') ||
        identifier.includes('float') ||
        identifier.includes('double')
      ) {
        return 0
  ...
  private handleMethodCall(
    ...
    if (
      ...
      collection !== null &&
      'seconds' in collection &&
      'nanoseconds' in collection
    ) {
    ...
    if (
      typeof collection === 'object' &&
      collection !== null &&
      ('hasValue' in collection ||
        'orValue' in collection ||
        'value' in collection)
    ) {
  ...
  private handleAllMethod(
    ...
    if (ctx.args.length === 1) {
      ...
      if (Array.isArray(collection)) {
        ...
        return this.evaluateAllForArrayWithIndex(
          collection,
          indexVariableExpr,
          valueVariableExpr,
          predicateExpr,
      ...
      // Handle maps (objects)
      if (typeof collection === 'object') {
        return this.evaluateAllForMapWithKey(
      ...
      throw new CelEvaluationError(
        'all() requires exactly two or three arguments',
      )
    }
  ...
  private evaluateAllForArray(
    ...
    if (extractedName) {
      ...
      throw new CelEvaluationError(
        'First argument to all() must be a variable identifier',
      )
    }
  ...
  private evaluateAllForArrayWithIndex(
    ...
    const indexVariableName = this.extractVariableName(
      indexVariableExpr,
      'all()',
    )
    const valueVariableName = this.extractVariableName(
      valueVariableExpr,
      'all()',
    )
  ...
  private handleExistsMethod(
    ...
    if (
      ...
      throw new CelEvaluationError(
        'exists() can only be called on lists or maps',
      )
    }
    ...
    if (ctx.args.length === 1) {
      ...
      if (Array.isArray(collection)) {
        ...
        return this.evaluateExistsForArray(
          collection,
          variableExpr,
          predicateExpr,
        )
      ...
      if (Array.isArray(collection)) {
        ...
        return this.evaluateExistsForArrayWithIndex(
          collection,
          indexVariableExpr,
          valueVariableExpr,
          predicateExpr,
      ...
      // Handle maps (objects)
      if (typeof collection === 'object') {
        return this.evaluateExistsForMapWithKey(
          collection as Record<string, unknown>,
          indexVariableExpr,
          valueVariableExpr,
          predicateExpr,
      ...
      throw new CelEvaluationError(
        'exists() requires exactly two or three arguments',
      )
    }
  ...
  private evaluateExistsForArray(
    ...
    if (extractedName) {
      ...
      throw new CelEvaluationError(
        'First argument to exists() must be a variable identifier',
      )
    }
  ...
  private evaluateExistsForMapWithKey(
    ...
    const keyVariableName = this.extractVariableName(
      keyVariableExpr,
      'exists()',
    )
    const valueVariableName = this.extractVariableName(
  ...
  private handleExistsOneMethod(
    ...
    if (
      ...
      throw new CelEvaluationError(
        'exists_one() can only be called on lists or maps',
      )
    }
    ...
    if (ctx.args.length === 1) {
      ...
      if (Array.isArray(collection)) {
        ...
        return this.evaluateExistsOneForArrayTwoParam(
          collection,
          variableExpr,
          predicateExpr,
        )
      ...
      if (Array.isArray(collection)) {
        ...
        return this.evaluateExistsOneForArray(
          collection,
          indexVariableExpr,
          valueVariableExpr,
          predicateExpr,
      ...
      // Handle maps (objects)
      if (typeof collection === 'object') {
        return this.evaluateExistsOneForMap(
          collection as Record<string, unknown>,
          indexVariableExpr,
          valueVariableExpr,
          predicateExpr,
      ...
      throw new CelEvaluationError(
        'exists_one() requires exactly two or three arguments',
      )
    }
  ...
  private handleFilterMethod(
    ...
    if (
      ...
      throw new CelEvaluationError(
        'filter() can only be called on lists or maps',
      )
    }
  ...
  private filterArray(
    ...
    if (extractedName) {
      ...
      throw new CelEvaluationError(
        'First argument to filter() must be a variable identifier',
      )
    }
  ...
  private handleMapMethod(
    ...
    if (
      !ctx.arg ||
      !ctx.args ||
      (ctx.args.length !== 1 && ctx.args.length !== 2)
    ) {
      throw new CelEvaluationError(
    ...
    if (isThreeArguments) {
      ...
      // Handle arrays
      if (Array.isArray(collection)) {
        return this.mapArrayWithFilter(
          collection,
          variableExpr,
          predicateExpr,
          transformExpr,
   ...
   */
  private isSimpleVariableReference(
    transformExpr: any,
    variableExpr: any,
  ): boolean {
  ...
  private handleSizeMethod(
    ...
    throw new CelEvaluationError(
      'size() can only be called on strings, lists, maps, or bytes',
    )
  }
  ...
  private handleTransformListMethod(
    ...
    if (!Array.isArray(collection)) {
      throw new CelEvaluationError(
        'transformList() can only be called on lists',
      )
    }
    ...
    if (
      !ctx.arg ||
      !ctx.args ||
      (ctx.args.length !== 2 && ctx.args.length !== 3)
    ) {
      throw new CelEvaluationError(
        'transformList() requires either three arguments (index, value, transform) or four arguments (index, value, filter, transform)',
      )
    }
    ...
    if (isFilterPresent) {
      ...
      return this.transformArrayWithFilter(
        collection,
        indexVariableExpr,
        valueVariableExpr,
        filterExpr,
  ...
  private handleTransformMapMethod(
    ...
    if (
      ...
      throw new CelEvaluationError(
        'transformMap() requires either three arguments (key, value, transform) or four arguments (key, value, filter, transform)',
      )
    }
    ...
    if (isFilterPresent) {
      ...
      const filterExpr = ctx.args[1]
      const transformExpr = ctx.args[2]
      return this.transformMapWithFilter(
        collection as Record<string, unknown>,
        keyVariableExpr,
        valueVariableExpr,
        filterExpr,
      ...
      const transformExpr = ctx.args[1]
      return this.transformMapObject(
        collection as Record<string, unknown>,
        keyVariableExpr,
        valueVariableExpr,
        transformExpr,
  ...
  private handleOptMapMethod(
    ...
    if (!ctx.arg || !ctx.args || ctx.args.length !== 1) {
      throw new CelEvaluationError(
        'optMap() requires exactly two arguments: variable and transform',
      )
    }
    ...
    throw new CelEvaluationError(
      'optMap() can only be called on optional values',
    )
  }
  ...
  private handleOptFlatMapMethod(
    ...
    if (!ctx.arg || !ctx.args || ctx.args.length !== 1) {
      throw new CelEvaluationError(
        'optFlatMap() requires exactly two arguments: variable and transform',
      )
    }
    ...
    throw new CelEvaluationError(
      'optFlatMap() can only be called on optional values',
    )
  }
  ...
  private handleStringSplit(
    ...
    if (ctx.args && ctx.args.length > 0) {
      ...
      if (typeof limit !== 'number' || !Number.isInteger(limit)) {
        throw new CelEvaluationError(
          'split() second argument must be an integer',
        )
      }
    }
  ...
  private handleStringIndexOf(
    ...
    if (ctx.args && ctx.args.length > 0) {
      ...
      if (typeof startIndex !== 'number' || !Number.isInteger(startIndex)) {
        throw new CelEvaluationError(
          'indexOf() second argument must be an integer',
        )
      }
  ...
  private handleStringLastIndexOf(
    ...
    if (!ctx.arg) {
      throw new CelEvaluationError(
        'lastIndexOf() requires at least one argument',
      )
    }
    ...
    if (typeof substring !== 'string') {
      throw new CelEvaluationError(
        'lastIndexOf() first argument must be a string',
      )
    }
    ...
    if (ctx.args && ctx.args.length > 0) {
      if (ctx.args.length > 1) {
        throw new CelEvaluationError(
          'lastIndexOf() takes at most two arguments',
        )
      }
      startIndex = this.visit(ctx.args[0])
      if (typeof startIndex !== 'number' || !Number.isInteger(startIndex)) {
        throw new CelEvaluationError(
          'lastIndexOf() second argument must be an integer',
        )
      }
  ...
  private handleStringReplace(
    ...
    if (ctx.args.length > 1) {
      ...
      if (typeof count !== 'number' || !Number.isInteger(count)) {
        throw new CelEvaluationError(
          'replace() third argument must be an integer',
        )
      }
    }
  ...
  private handleStringSubstring(
    ...
    if (typeof start !== 'number' || !Number.isInteger(start)) {
      throw new CelEvaluationError(
        'substring() first argument must be an integer',
      )
    }
        ...
        switch (specifier) {
          ...
          case 'd':
            ...
            if (
              arg &&
              typeof arg === 'object' &&
              (arg as any).__isFloatLiteral !== undefined
            ) {
          ...
          case 'x':
            if (typeof arg === 'number') {
              if (!isFinite(arg)) {
                throw new CelEvaluationError(
                  'only integers, byte buffers, and strings can be formatted as hex, was given double',
                )
              }
          ...
          case 'E':
            ...
            throw new CelEvaluationError(
              `could not parse formatting clause: unrecognized formatting clause "${specifier}"`,
            )
          default:
            throw new CelEvaluationError(
              `could not parse formatting clause: unrecognized formatting clause "${specifier}"`,
  ...
  private formatValueForString(value: any): string {
    ...
    if (
      value &&
      typeof value === 'object' &&
      '__isDynamic' in value &&
      value.__isDynamic === true
    ) {
  ...
  private checkForProtobufObject(value: any): void {
    if (value && typeof value === 'object') {
      ...
      if (
        value.repeated_nested_enum !== undefined &&
        value.standalone_enum !== undefined
      ) {
        throw new CelEvaluationError(
          `error during formatting: string clause can only be used on strings, bools, bytes, ints, doubles, maps, lists, types, durations, and timestamps, was given cel.expr.conformance.proto3.TestAllTypes`,
        )
      ...
      if (
        ...
        throw new CelEvaluationError(
          `error during formatting: string clause can only be used on strings, bools, bytes, ints, doubles, maps, lists, types, durations, and timestamps, was given ${value.constructor.name}`,
        )
      }
    }
  }
  ...
  private handleTimestampMethod(
    ...
    if (
      ...
      if (typeof timezoneArg !== 'string') {
        throw new CelEvaluationError(
          `${methodName}() timezone argument must be a string`,
        )
      }
    ...
    const dateToUse = timezone
      ? this.applyTimezone(timestamp, timezone)
      : timestamp
      ...
      case 'getDayOfYear':
        ...
        if (timezone) {
          ...
          const startOfYear = new Date(
            Date.UTC(adjustedDate.getUTCFullYear(), 0, 1),
          )
          return Math.floor(
            (adjustedDate.getTime() - startOfYear.getTime()) /
              (24 * 60 * 60 * 1000),
          )
        } else {
          const startOfYear = new Date(
            Date.UTC(timestamp.getUTCFullYear(), 0, 1),
          )
          const dayOfYear = Math.floor(
            (timestamp.getTime() - startOfYear.getTime()) /
              (24 * 60 * 60 * 1000),
          )
          return dayOfYear
        }
